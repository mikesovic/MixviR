...
#' @param read.muts.from By default, data are read from *samp_mutations* data frame that is created by `call_mutations()` and written to the global environment. If this data frame has been written to a file (see *write.mut.table* in `call_mutations()`), the dashboard can optionally be opened by providing the path to that file. The fields "SAMP_NAME, CHR, POS, GENE, ALT_ID, AF, DP" must be present (additional fields will be ignored).
#' @param all.target.muts Logical to indicate whether results for all target mutations were written to the output of the `call_mutations()` function. See *write.all.targets* option in `call_mutations()`. Default FALSE. If TRUE, more informative sequencing depth information can be provided in dashboard.
#' @keywords shiny
...
explore_mutations <- function(dates = NULL, 
                              lineage.muts = NULL, 
                              read.muts.from = NULL,
